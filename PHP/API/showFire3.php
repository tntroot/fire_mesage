...
/* $Type =SQL_Injection_Prevention('強化液');
$Supplier =SQL_Injection_Prevention('科比');
...
if(is_numeric($Page)){
    switch($Page){
        case "0":
            if($Type & $Supplier){
                try{
                    $stmt = $pdo->prepare(prepare_showFire3(1,0));
                ...
                try{
                    $stmt = $pdo->prepare(prepare_showFire3(2,0));
                    $stmt->execute([$Type]);
                }catch(Exception $e){
                    return_error_and_exit('fail', '3', $e,$DEBUG); 
                }
                ...
                try{
                    $stmt = $pdo->prepare(prepare_showFire3(3,0));
                ...
                try{
                    $stmt = $pdo->prepare(prepare_showFire3(4,0));
                    $stmt->execute();
                }catch(Exception $e){
        ...
        default:
            ...
            if($Type & $Supplier){
                try{
                    $stmt = $pdo->prepare(prepare_showFire3(1,1));
                ...
                try{
                    $stmt = $pdo->prepare(prepare_showFire3(2,1));
                ...
                try{
                    $stmt = $pdo->prepare(prepare_showFire3(3,1));
                ...
                try{
                    $stmt = $pdo->prepare(prepare_showFire3(4,1));
                    $stmt->execute([$Page]);
                }catch(Exception $e){
                    return_error_and_exit('fail', '4', $e,$DEBUG); 
                }   
